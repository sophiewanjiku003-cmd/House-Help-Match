...
class UserProfileForm(forms.ModelForm):
    class Meta:
        model = UserProfile
        fields = ['user_type', 'phone', 'address', 'city', 'profile_pic']
        widgets = {
            'address': forms.Textarea(attrs={'rows': 3, 'placeholder': 'Enter your full address'}),
        }

class HouseHelpProfileForm(forms.ModelForm):
    class Meta:
        model = HouseHelpProfile
        fields = ['skills', 'experience', 'skill_level', 'hourly_rate', 'bio', 'is_available']
        widgets = {
...
class JobPostingForm(forms.ModelForm):
    category = forms.ModelMultipleChoiceField(
        ...
        required=True,
        help_text='Select one or more categories that apply to this job'
    )
